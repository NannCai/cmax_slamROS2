...
public:
    // Constructor
    AngVelEstimator(rclcpp::Node::SharedPtr node);
    ...
    void pushEvent(const DvsEvent& event);
...
//     ros::NodeHandle* nh_;
    ...
    rclcpp::Publisher<geometry_msgs::msg::TwistStamped>::SharedPtr ang_vel_pub_;
    ...
    // Slide window
    std::vector<DvsEvent> event_subset_; // Event subset for the current window
    ...
    rclcpp::Duration dt_av_ = rclcpp::Duration::from_seconds(0); // Frequency of output angular velocity
